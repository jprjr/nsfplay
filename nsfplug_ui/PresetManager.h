...
public:
  ...
  bool CreatePreset(ini::NSFPlayerConfig *, const CString &name );
  bool LoadPreset(ini::NSFPlayerConfig *, const CString &name);
  const std::set<CString> GetPresetNames();
  const std::set<CString> GetPresetSects();
  bool SavePreset(ini::NSFPlayerConfig *, const CString &name);
